...
# Домашнее задание к занятию 4 «`Оркестрация группой Docker контейнеров на примере Docker Compose`»`Скворцов Денис`
...
```
Из-за того что родительским процессом c PID = 1 выступает nginx,
а при подключении мы получили доступ к родительскому процессу контейнера, то отправка
сигнала `SIGINT` нажатием Ctrl+C прерывает процесс Nginx, вследствие чего контейнер останавливается
```
...
5. Откройте страницу "http://127.0.0.1:9000/#!/home", выберите ваше local  окружение. Перейдите на вкладку "stacks" и в "web editor" задеплойте следующий compose:
